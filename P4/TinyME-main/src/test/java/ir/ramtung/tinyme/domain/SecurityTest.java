...
class SecurityTest {
    ...
    void setupOrderBook() {
        ...
        orders = Arrays.asList(
                ...
                new Order(11, security, Side.SELL, 400, 15900, 100, broker, shareholder)
        );
    ...
    void increasing_iceberg_peak_size_changes_priority() {
        ...
        orders = Arrays.asList(
                new Order(1, security, BUY, 304, 15700, 0, broker, shareholder),
                new Order(2, security, BUY, 43, 15500, 0, broker, shareholder),
                new IcebergOrder(3, security, BUY, 445, 15450, 0, broker, shareholder, 100),
                new Order(4, security, BUY, 526, 15450, 0, broker, shareholder),
                new Order(5, security, BUY, 1000, 15400, 0, broker, shareholder)
        );
        orders.forEach(order -> security.getOrderBook().enqueue(order));
    ...
    void update_iceberg_that_loses_priority_with_no_trade_works() {
        ...
        security.getOrderBook().enqueue(
                new IcebergOrder(1, security, BUY, 100, 9, 0, broker, shareholder, 10)
        );
    ...
    void update_iceberg_order_decrease_peak_size() {
        security = Security.builder().isin("TEST").build();
        security.getOrderBook().enqueue(
                new IcebergOrder(1, security, BUY, 20, 10, 0, broker, shareholder, 10)
        );
    ...
    void update_iceberg_order_price_leads_to_match_as_new_order() throws InvalidRequestException {
        ...
        orders = List.of(
                new Order(1, security, BUY, 15, 10, 0, broker, shareholder),
                new Order(2, security, BUY, 20, 10, 0, broker, shareholder),
                new Order(3, security, BUY, 40, 10, 0, broker, shareholder),
                new IcebergOrder(4, security, SELL, 30, 12, 0, broker, shareholder, 10)
        );
        orders.forEach(order -> security.getOrderBook().enqueue(order));
