...
class SecurityTest {
    ...
    void increasing_quantity_changes_priority() {
        EnterOrderRq updateOrderRq = EnterOrderRq.createUpdateOrderRq(1, security.getIsin(), 3, LocalDateTime.now(), BUY, 450, 15450, 0, 0, 0, 0, 0, true);
        assertThatNoException().isThrownBy(() -> security.updateOrder(updateOrderRq, matcher));
    ...
    void changing_price_changes_priority() {
        EnterOrderRq updateOrderRq = EnterOrderRq.createUpdateOrderRq(1, security.getIsin(), 1, LocalDateTime.now(), BUY, 300, 15450, 0, 0, 0, 0, 0, true);
        assertThatNoException().isThrownBy(() -> security.updateOrder(updateOrderRq, matcher));
    ...
    void updating_non_existing_order_fails() {
        EnterOrderRq updateOrderRq = EnterOrderRq.createUpdateOrderRq(1, security.getIsin(), 6, LocalDateTime.now(), BUY, 350, 15700, 0, 0, 0, 0, 0, true);
    ...
    void trying_to_change_minimum_execution_quantity_fails(){
        EnterOrderRq updateOrderRq = EnterOrderRq.createUpdateOrderRq(1, security.getIsin(), 11, LocalDateTime.now(), Side.SELL, 400, 15600, 0, 0, 0, 0, 0, true);
        assertThatExceptionOfType(InvalidRequestException.class).isThrownBy(() -> security.updateOrder(updateOrderRq, matcher));
    }
    ...
    void deleting_non_existing_order_fails() {
        DeleteOrderRq deleteOrderRq = new DeleteOrderRq(1, security.getIsin(), Side.SELL, 1, 0, true);
    ...
    void increasing_iceberg_peak_size_changes_priority() {
        ...
        orders = Arrays.asList(
                new Order(1, security, BUY, 304, 15700, 0, broker, shareholder, 0),
                new Order(2, security, BUY, 43, 15500, 0, broker, shareholder, 0),
                new IcebergOrder(3, security, BUY, 445, 15450, 0, broker, shareholder, 100, 0),
                new Order(4, security, BUY, 526, 15450, 0, broker, shareholder, 0),
                new Order(5, security, BUY, 1000, 15400, 0, broker, shareholder, 0)
        );
        orders.forEach(order -> security.getOrderBook().enqueue(order));
        EnterOrderRq updateOrderRq = EnterOrderRq.createUpdateOrderRq(1, security.getIsin(), 3, LocalDateTime.now(), BUY, 445, 15450, 0, 0, 150, 0, 0, true);
        assertThatNoException().isThrownBy(() -> security.updateOrder(updateOrderRq, matcher));
    ...
    void decreasing_iceberg_quantity_to_amount_larger_than_peak_size_does_not_changes_priority() {
        ...
        orders.forEach(order -> security.getOrderBook().enqueue(order));
        EnterOrderRq updateOrderRq = EnterOrderRq.createUpdateOrderRq(1, security.getIsin(), 3, LocalDateTime.now(), BUY, 300, 15450, 0, 0, 100, 0, 0, true);
        assertThatNoException().isThrownBy(() -> security.updateOrder(updateOrderRq, matcher));
        assertThat(security.getOrderBook().getBuyQueue().get(2).getOrderId()).isEqualTo(3);
    }
    ...
    void update_iceberg_that_loses_priority_with_no_trade_works() {
        ...
        security.getOrderBook().enqueue(
                new IcebergOrder(1, security, BUY, 100, 9, 0, broker, shareholder, 10, 0)
        );

        EnterOrderRq updateReq = EnterOrderRq.createUpdateOrderRq(2, security.getIsin(), 1, LocalDateTime.now(), BUY, 100, 10, 0, 0, 10, 0, 0, true);
    ...
    void update_iceberg_order_decrease_peak_size() {
        security = Security.builder().isin("TEST").build();
        security.getOrderBook().enqueue(
                new IcebergOrder(1, security, BUY, 20, 10, 0, broker, shareholder, 10, 0)
        );

        EnterOrderRq updateReq = EnterOrderRq.createUpdateOrderRq(2, security.getIsin(), 1, LocalDateTime.now(), BUY, 20, 10, 0, 0, 5, 0, 0, true);
        assertThatNoException().isThrownBy(() -> security.updateOrder(updateReq, matcher));
    ...
    void update_iceberg_order_price_leads_to_match_as_new_order() throws InvalidRequestException {
        ...
        orders = List.of(
                new Order(1, security, BUY, 15, 10, 0, broker, shareholder, 0),
                new Order(2, security, BUY, 20, 10, 0, broker, shareholder, 0),
                new Order(3, security, BUY, 40, 10, 0, broker, shareholder, 0),
                new IcebergOrder(4, security, SELL, 30, 12, 0, broker, shareholder, 10, 0)
        );
        orders.forEach(order -> security.getOrderBook().enqueue(order));
